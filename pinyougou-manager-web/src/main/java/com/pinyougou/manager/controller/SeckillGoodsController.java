...
public class SeckillGoodsController {
	...
	public Result updateStatus(@RequestBody Long[] ids,String status) {
		try {
			...
			//发送消息，审核商品要进行页面生成
			MessageInfo messageInfo = new MessageInfo("TOPIC_SECKILL", "Tags_genHtml", "seckillGoods_updateStatus"
					, ids, MessageInfo.METHOD_ADD);
			producer.send(new Message(messageInfo.getTopic(),messageInfo.getTags(),
					messageInfo.getKeys(), JSON.toJSONString(messageInfo).getBytes(RemotingHelper.DEFAULT_CHARSET)));
			return new Result(true,"修改成功");
		} catch (Exception e) {
			e.printStackTrace();
			return new Result(false,"修改失败");
