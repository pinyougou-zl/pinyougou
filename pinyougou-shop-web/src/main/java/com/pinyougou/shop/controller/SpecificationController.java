...
public class SpecificationController {
	...
	public Result update(@RequestBody Specification specification){
		try {
			...
			specificationService.updateApply(specification);
	...
	public Specification findOne(@PathVariable(value = "id") Long id){
		...
		return specificationService.oneFindOne(id);
	}
    ...
    public PageInfo<TbSpecification> findPage(@RequestParam(value = "pageNo", defaultValue = "1", required = true) Integer pageNo,
        ...
        return specificationService.oneFindPage(pageNo, pageSize, specification);
    }
	@RequestMapping("/addApply")
	public Result addApply(@RequestBody Specification specification){
		try {
			for (TbSpecificationOption option : specification.getOptionList()) {
				option.setStatus("0");
			}
